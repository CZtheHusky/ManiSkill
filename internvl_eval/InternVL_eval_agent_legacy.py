...
def generate_prefix_fn(numbers_list, start_list, end_list, connect_list):
...
class InternVLEvalAgent:
    def __init__(
        ...
        for str_ in start_sign:
            toks = self.tokenizer.tokenize(str_)
            assert len(toks) == 1
            start_list.append(self.tokenizer.convert_tokens_to_ids(toks)[0])
        for str_ in end_sign:
            toks = self.tokenizer.tokenize(str_)
            assert len(toks) == 1
            end_list.append(self.tokenizer.convert_tokens_to_ids(toks)[0])
        for str_ in connect_sign:
            toks = self.tokenizer.tokenize(str_)
            assert len(toks) == 1
            connect_list.append(self.tokenizer.convert_tokens_to_ids(toks)[0])
        for str_ in numbers:
            toks = self.tokenizer.tokenize(str(str_))
            assert len(toks) == 1
            numbers_list.append(self.tokenizer.convert_tokens_to_ids(toks)[0])
        prefix_processor = SafePrefixConstrainedLogitsProcessor(
                prefix_allowed_tokens_fn=generate_prefix_fn(numbers_list, start_list, end_list, connect_list),
                num_beams=1,
            )
        processor_list = LogitsProcessorList([
            prefix_processor,
        ])
        self.generation_config = dict(max_new_tokens=15, do_sample=True, logits_processor=processor_list)
...
if __name__ == "__main__":    
    ...
    tasks_to_run = [
        ["/home/caozhe/workspace/vlav-project/maniskill_stack_cubes_dual/internvl2-2b/v0-20250729-171130", "checkpoint-1280", 7]
    ]
